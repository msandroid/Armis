...
export const SearchResults: React.FC<SearchResultsProps> = ({
  ...
  const getRelevanceLabel = (relevance: number) => {
    if (relevance >= 0.9) return '高'
    if (relevance >= 0.7) return '中'
    return '低'
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-600 dark:text-gray-400">
              総結果数: {totalResults.toLocaleString()}
            </span>
          </div>
          <div className="flex items-center space-x-2">
            <Clock className="w-4 h-4 text-teal-500" />
            <span className="text-sm text-gray-600 dark:text-gray-400">
              検索時間: {searchTime.toFixed(2)}秒
            </span>
          </div>
          <div className="flex items-center space-x-2">
            <Search className="w-4 h-4 text-teal-500" />
            <span className="text-sm text-gray-600 dark:text-gray-400">
              表示結果: {results.length}件
            </span>
          </div>
        </div>
      </div>
          ...
          <div key={index} className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-gray-400 dark:text-gray-500 ml-2">
                関連性: {Math.round(result.relevance * 100)}%
              </span>
            </div>
          </div>
        ))}
