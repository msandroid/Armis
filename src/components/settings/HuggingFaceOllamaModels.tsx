...
import { HuggingFaceOllamaService, HuggingFaceOllamaModel } from '@/services/llm/huggingface-ollama-service'

interface HuggingFaceOllamaModelsProps {
  llmManager: any
  onModelPull?: (modelName: string) => void
}

export const HuggingFaceOllamaModels: React.FC<HuggingFaceOllamaModelsProps> = ({
  llmManager,
  onModelPull
}) => {
  const [models, setModels] = useState<HuggingFaceOllamaModel[]>([])
  const [trendingModels, setTrendingModels] = useState<HuggingFaceOllamaModel[]>([])
  const [popularModels, setPopularModels] = useState<HuggingFaceOllamaModel[]>([])
  ...
  const huggingFaceService = new HuggingFaceOllamaService()
  ...
  const loadModels = async () => {
    ...
    try {
      let fetchedModels: HuggingFaceOllamaModel[] = []

      if (selectedFamily === 'all') {
        // 全モデルを取得
        fetchedModels = await huggingFaceService.searchOllamaModels({
          query: searchQuery,
          limit: 50,
          sort: sortBy,
          direction: 'desc'
      ...
      const [trending, popular] = await Promise.all([
        huggingFaceService.getTrendingOllamaModels(20),
        huggingFaceService.getPopularOllamaModels(20)
      ])
  ...
  const renderModelCard = (model: HuggingFaceOllamaModel) => (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <span>Hugging Face Ollama Models</span>
            <Badge variant="outline">Live</Badge>
          </CardTitle>
          <CardDescription>
            Hugging FaceからOllama対応モデルを検索してダウンロードできます。
          </CardDescription>
        </CardHeader>
